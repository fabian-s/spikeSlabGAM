...
probability of \Sexpr{round(summ[[1]]["111110010100000000000000000010010000"],2)}. The correct model
that additionally includes \code{lin1} (column \Sexpr{which(names(summ[[1]])=="111111010100000000000000000010010000")})
has a posterior probability of about \Sexpr{round(summ[[1]]["111111010100000000000000000010010000"],3)}.}
\label{sum12}
\end{figure}
...
@
\caption{Posterior means and pointwise 80\% credible intervals for model \texttt{m}.
...
@
<<plotm1sm2fprep, echo=FALSE, cache=TRUE>>=
trueFsm2f <-data.frame(truth=fsm2f-mean(fsm2f), sm2=sm2, f=f)
